...
A number of researchers have worked to overcome the limitations (e.g., less-than-ideal accuracy, lack of availability indoors) of GPS technology for assisting people who are \BVI with \OM.  Researchers have pursued roughly two approaches.  The first is to utilize crowdsourcing, whereby people who are \BVI connect over the internet with a sighted person for on-demand assistance.  Examples of this approach include the VizWiz project \cite{bigham2010vizwiz}, BeMyEyes \cite{bemyeyesaccessworld}, and Aira \cite{aira} (both BeMyEyes and Aira support a video chat interface).  A second approach combines rudimentary motion estimates derived from inertial sensors (gyroscopes and accelerometers) with detection of fixed environmental infrastructure (e.g., Bluetooth location beacons, Wifi access points).  For instance, \cite{ganz2015percept, ganz2011percept, ganz2014percept} developed a system for navigating indoors using smartphone-detectable RFID tags.  Dias and her collaborators utilized WiFi fingerprinting (calibrated using a highly-precise indoor mapping robot) and dead-reckoning for indoor navigation \cite{Dias__2014_7778}.  A similar system uses low-energy Bluetooth beacons instead of WiFi access points to improve localization \cite{ishihara2017beacon, ahmetovic2016navcog, ahmetovic2017achieving}.  Another work utilizes a database containing images at known locations, which are matched to camera images at run time \cite{bai2014wearable}.\todo{figure out a place to integrate some of Aaron Steinfeld's work}

Another area of active research is the development of solutions that help people who are \BVI find objects whose locations were never known, were dropped or misplaced, or were moved by a third party.  While the task of object finding doesn't traditionally fall under the umbrella of \OM, here we stretch the terminology to encompass it by noting that both \OM and object finding require a significant degree of spatial awareness.  As in the case of navigation, crowdsourcing-based technologies have shown promise for object finding.  Other approaches use automatic object detection and tracking as a means to both find and navigate to objects \cite{schauerte2012assistive, jafri2014computer, thakoor2014system}.\todo{might need to explicitly contrast with our work, why is ours better, or just say that we will discuss this later}
...
Recently, both Apple and Google have released support for highly sophisticated smartphone-based AR experiences, whereby virtual and real content are combined (e.g., by overlaying virtual objects or characters on a smartphone's camera feed).  For instance, an app might show a virtual cat projected into a real world scene.  As the user moves around in space, the phone senses the user's motion with high accuracy and renders the cat at an appropriate distance and viewing angle, providing the illusion that the cat exists in the physical world.
...
To understand the potential of VIO for creating assistive \OM apps, it helps to understand a bit about how these algorithms work.  A full explanation of VIO is beyond the scope of this document (see, e.g., \cite{gui2015review}).  Approaches for VIO have been developed for both the stereo and the monocular (single camera) settings.  The monocular setting is applicable to most smartphones, which rarely have stereo cameras.  For the remainder of this paper, VIO refers to the monocular setting.

VIO algorithms utilize sensor fusion to blend optical and inertial motion estimates.  Optical motion estimates are made by tracking salient visual features (for instance, corners or other highly textured portions of the image) through multiple video frames.  Utilizing the mathematics of perspective geometry, one can estimate the rotation and translation of the phone \cite{Hartley2004}.  Of particular importance, the accuracy of these estimates is dependent on tracking a large number of visual features that should, ideally, correspond to points at a range of depths from the camera and be distributed uniformly across the image.  While some environments are intrinsically more difficult for optical tracking, we have observed that users may exacerbate this difficulty by holding their phones in a suboptimal orientation (e.g., with the camera facing the ground).
...
\subsection{Usability Factors of AR for \OM Apps}
Since AR relies on VIO, which is, in part, on optical tracking technique, any assistive app based on AR requires that the user leave their smartphone's camera unoccluded.  Here, we assume that the user holds the phone in one hand while holding their long cane in the other.  In some cases the user may require handsfree operation of their smartphone (handsfree operation is an area we are actively researching).
...
We present \emph{Clew}, an iPhone app based on the the ARKit platform (link to Clew on the app store redacted for anonymous review).  Clew provides turn-by-turn directions to guide users who are \BVI along previously traveled routes.  This use case arises, primarily, in two scenarios.  First, when a user is led somewhere unfamiliar by a sighted guide, they may desire to return to their previous location at a later time --- without the need for further human assistance.  Secondly, in some situations it is easier for someone who is \BVI to navigate from a location than back to it.  For instance, it is easier to leave a conference room than it is to find your way back to your particular seat.  In both of these uses cases, \emph{Clew} provides high-accuracy, easy-to-follow, navigational guidance to enable people who are \BVI to travel independently indoors.  Importantly, Clew requires no modifications to the environment (e.g., the introduction of beacons or special signage).  
...
The ViewShare app generates haptic feedback when the phone is oriented towards a localized object.  As mentioned earlier, the low latency motion updating provided by ARKit allows a user to sweep the phone and easily determine the direction of objects.  Further, the app synthesizes speech feedback to announce both the name of the object and its distance from the user.  The app can provide feedback in two modes.  The first mode is \emph{2D feedback} in which distance and orientation to an object are determined by projecting 3D spatial information into the floor plane.  This mode is useful for navigating to objects that are far away (since the height of the object is irrelevant until the user gets close).  The second mode is \emph{3D feedback} in which distance and orientation to an object are determined using the unmodified 3D positions and orientations of the phone and localized objects.  This mode is appropriate for finding objects that are close and relatively small in terms of their vertical extant.  As an example to motivate the difference between these modes, 3D mode is suited to finding a pen, whereas 2D mode is suited to finding a door.
